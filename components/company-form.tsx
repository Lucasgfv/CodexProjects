...
export function CompanyForm({ action, company }: { action: CompanyAction; company?: Company }) {
  ...
  return <form ref={formRef} action={formAction} className="form-card" encType="multipart/form-data">
    ...
    <section className="form-section"><h2 className="section-title">Identificação e situação cadastral</h2><div className="form-grid">
      ...
      <div className="field"><label htmlFor="dataAbertura">Data de abertura (Cartão CNPJ)</label><input id="dataAbertura" type="date" name="dataAbertura" defaultValue={iso(company, "dataAbertura")} {...errorProps("dataAbertura")} /><ErrorMessage state={state} name="dataAbertura" /></div>
    ...
    <section className="form-section"><h2 className="section-title">Atividade e quadro operacional</h2><div className="form-grid">
      ...
      <div className="field span-6"><label htmlFor="cnaesSecundarios">CNAEs secundários</label><textarea id="cnaesSecundarios" name="cnaesSecundarios" placeholder="Cole ou digite livremente, um por linha" defaultValue={Array.isArray(company?.cnaesSecundarios) ? company.cnaesSecundarios.join("\n") : ""} /></div>
    ...
    <section className="form-section"><h2 className="section-title">Relacionamento com o escritório</h2><div className="form-grid">
      <div className="field"><label htmlFor="tipoCliente">Tipo de cliente</label><select id="tipoCliente" name="tipoCliente" defaultValue={value(company, "tipoCliente") || "FIXO"} {...errorProps("tipoCliente")}><option value="FIXO">Fixo</option><option value="AVULSO">Avulso</option><option value="IRPF">IRPF</option></select><ErrorMessage state={state} name="tipoCliente" /></div>
